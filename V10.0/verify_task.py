...
chk("Deployment config present",
    ...
    and (root.parent / "railway.json").exists()
    and (root.parent / ".devcontainer" / "devcontainer.json").exists(),
    "Dockerfile + Railway config (repo root) + Codespaces devcontainer")

# The live deployment, asserted rather than described. This is the link that goes in the
# submission form, so a broken one should fail the check that claims the build is done.
def _live_service() -> tuple[bool, str]:
    # `requests`, not `urllib`. urllib trusts the OS certificate store, and a machine with
    # an expired root there reports CERTIFICATE_VERIFY_FAILED against a perfectly healthy
    # service — which is exactly what happened here, and a check that fails on a working
    # deployment is worse than no check. requests ships certifi and is already a dependency.
    import requests

    url = "https://ragoa-voice-rag.up.railway.app/health"
    try:
        body = requests.get(url, timeout=25).json()
    except Exception as exc:  # noqa: BLE001
        return False, f"{url} unreachable: {type(exc).__name__}: {exc}"
    ok = body.get("ok") is True and body.get("index_size", 0) > 0
    return ok, (f"index_size={body.get('index_size')} stt={body.get('stt_configured')} "
                f"generation={body.get('generation_enabled')}")

_live_ok, _live_detail = _live_service()
chk("Live deployment answers /health", _live_ok, _live_detail)
