...
**Live on Railway: https://ragoa-voice-rag.up.railway.app/**

Everything below is the search that led there, kept because the measurements rule out most
of the obvious free options and the reasoning is worth having on record. The Cloudflare
tunnel and Codespaces paths still work and are still documented — they are the fallback if
the Railway credit runs out.
...
## Railway free trial — 1 GB, no card *(this is what shipped)*
...
2. Settings -> Source -> Root Directory  ->  V9.0        <- do this before the first build
3. Variables -> add SARVAM_API_KEY
...
### Step 2 is the one that actually matters

Skipping it fails the build on the very first `COPY`:

```
failed to compute cache key: "/requirements.txt": not found
```

Railway has no build-context setting. `dockerContext` is **not** a field in its
Config-as-Code schema, and an unknown key is ignored rather than rejected — so adding one
changes nothing while looking like it should have worked. The build context is always
whatever directory Railway checks out, and **Root Directory** is the only thing that
controls that. It is dashboard-only; it cannot be expressed in `railway.json`.

Once Root Directory is `V9.0`, `dockerfilePath` has to come *out* of `railway.json` —
Railway auto-detects a `Dockerfile` at the root of the scoped source, and a leftover
`"V9.0/Dockerfile"` makes it look for `V9.0/V9.0/Dockerfile` and fail a second way.

Watch Paths is unrelated. It only decides whether a push triggers a redeploy.
