...
class GuardrailSuite:
    ...
    def run_pre_retrieval(
        self, query: str, query_vector: np.ndarray | None = None
    ) -> list[GuardrailVerdict]:
        """Runs before the query is even embedded. Short-circuits on the first block.

        `query_vector` is accepted but optional, and the orchestrator passes nothing:
        neither of these checks needs an embedding, and requiring one would force a ~90ms
        embed before a ~0.05ms refusal. The parameter stays so a future guardrail that
        does need the vector can be added here without changing the call site — it would
        just have to run after the embed.
        """
