...
class TextQuery(BaseModel):
    ...
    #: Opt-in English to Hindi translation before retrieval. Off by default -- see
    #: translation/translator.py for why this is never automatic.
...
def ask_text(body: TextQuery) -> dict:
    ...
    response = get_pipeline().run_text(
        body.query, top_k=body.top_k, translate=body.translate
    )
...
def speak(body: SpeakRequest) -> Response:
    """Synthesise Hindi speech and stream it back as a WAV.

    Used for both halves of the loop: reading the transcribed question back, and reading
    the answer aloud. The browser plays the bytes directly, so no file ever lands on disk.
    """
    from tts.speaker import Speaker

    result = Speaker().speak(body.text)
    if not result.ok:
        raise HTTPException(status_code=502, detail=str(result.error))

    return Response(
        content=result.value.wav_bytes,
        media_type="audio/wav",
        headers={
            "Cache-Control": "no-store",
            "X-TTS-Segments": str(result.value.segments),
            "X-TTS-Elapsed-Ms": f"{result.elapsed_ms:.0f}",
        },
    )
