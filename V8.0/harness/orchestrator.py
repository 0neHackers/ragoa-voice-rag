...
from harness.types import (
    ...
    ResponseStatus, RetrievalResult, Stage, StageError, StageResult, Timer,
    Transcript, TranslationInfo,
)
...
class VoiceRAGPipeline:
    ...
    translator: Any = None              # translation.translator.Translator
    ...
    def run_text(
        ...
        *,
        ...
        # ---- optional English to Hindi translation ------------------------
        # Runs before everything, guardrails included, because it changes what those
        # guardrails are judging. Opt-in only: auto-translating would silently disable
        # `language_mismatch` -- which exists because of a measured failure -- and would
        # mangle romanised Hindi that is already a Hindi question.
        translation_info: TranslationInfo | None = None
        if translate and self.translator is not None:
            translate_result = self._run_stage(
                Stage.TRANSLATE, lambda: self.translator.translate(query), retries=1
            )
            latency.record(Stage.TRANSLATE, translate_result.elapsed_ms)
            latency.translation_ms = translate_result.elapsed_ms

            if not translate_result.ok:
                # Falling through with the English text would trip `language_mismatch`
                # and produce a confusing refusal. Report the real cause instead.
                return PipelineResponse.failed(
                    translate_result.error, query=query,
                    trace_id=trace_id, latency=latency,
                )

            translated = translate_result.value
        ...
        if not self.config.enable_generation or self.generator is None:
            ...
            return PipelineResponse(
                ...
                guardrails=verdicts, latency=latency, trace_id=trace_id,
                reason="retrieval_only", translation=translation_info,
            )
        ...
        if not gen_result.ok:
            ...
            return PipelineResponse(
                ...
                latency=latency, trace_id=trace_id, translation=translation_info,
            )
        ...
        if self.config.enable_guardrails and self.guardrails is not None:
            ...
            if not grounded.passed:
                ...
                return PipelineResponse.declined(
                    ...
                    guardrails=verdicts, latency=latency, trace_id=trace_id,
                    answer_mode=answer.mode, translation=translation_info,
                )
        ...
        return PipelineResponse(
            ...
            answer_mode=answer.mode, trace_id=trace_id, translation=translation_info,
        )
