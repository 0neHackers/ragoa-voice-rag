...
class TestGroundedness:
    def test_grounded_answer_passes(self):
        ...
        assert GroundednessGuardrail(min_overlap=0.20).run("q", answer, _retrieval(_scored(context, 0.8))).passed
    ...
    def test_hallucinated_answer_blocked(self):
        ...
        verdict = GroundednessGuardrail(min_overlap=0.20).run("q", answer, _retrieval(_scored(context, 0.8)))
        assert not verdict.passed
    ...
    def test_stopwords_do_not_inflate_overlap(self):
        ...
        answer = Answer(text="the quantum chromodynamics of gluon confinement in lattice",
                        mode="generated", model="test")
        assert not GroundednessGuardrail(min_overlap=0.20).run("q", answer, _retrieval(_scored(context, 0.8))).passed
